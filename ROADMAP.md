...
- Mini absorb registry + scalars (v1.2.164): book `pushSync` REPLACE still takes satellite qty / extras, then `overlayMiniBookAfterRemoteReplace` keeps a Mini-created Kids book, a Mini delete / rename, and Mini-edited staking / FIRE / budgets · satellite dirty pull overlays the same scalars (`overlaySatelliteNonCollectionFields`) so an iPad reward is not wiped then pushed · dirty overlay drops Mini-deleted holdings so a MacBook channel does not resurrect SOL · `#F7931A` + Mini-as-book kept
