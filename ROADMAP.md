...
**Current version: 1.2.165**

## Completed (through v1.2.165)

- Landscape nav clearance + 44px taps + CHARTS wrap (v1.2.165): short-landscape shell is 100dvh with `.app-main` padding ≥ nav height (4.25rem + 0.5rem + safe-area) so SIPP/liabilities, MIX, and Encrypted cloud sync cannot paint under the bar · header menu / Refresh / More are 44×44 · Today jump chips wrap at 390 so CHARTS is fully readable · `#F7931A` + Mini-as-book kept · draft only — do not merge #206
- Mini absorb registry + scalars (v1.2.164): book `pushSync` REPLACE still takes satellite qty / extras, then `overlayMiniBookAfterRemoteReplace` keeps a Mini-created Kids book (including an already-pushed Kids / ETH / staking reward / Kids → Children a stale satellite extras PUT omitted via `restoreMiniKeptBooks` + `restoreMiniKeptHoldings` + `restoreMiniKeptScalars` + `restoreMiniKeptNames` + envelope `lastPulledBookIds` / `lastPulledHoldings` / `lastPulledScalarHashes` / `lastPulledRegistryNames`; a satellite that pulled Kids / SOL and deleted it, or edited staking / renamed, still lands; `mydsp_last_pulled_holding_ids` survives a Settings save), a Mini delete / rename, and Mini-edited staking / FIRE / budgets · `overlayMiniLiveMarks` keeps Mini live prints so a MacBook qty does not snap prices back · merge absorb drops a satellite SOL delete Mini did not edit (`dropUneditedRowsDeletedOnRemote`) · satellite dirty pull overlays the same scalars (`overlaySatelliteNonCollectionFields`) so an iPad reward is not wiped then pushed · dirty overlay drops Mini-deleted holdings so a MacBook channel does not resurrect SOL · Unlock / reload keeps an unpushed satellite qty via `satelliteBookDivergedFromLastPull` + `overlaySatelliteBookAfterRemoteReplace` (`mydsp_last_pulled_holding_hashes`) · an unpushed YouTube / News / Markets edit uses `satelliteExtrasDivergedFromLastPull` (`mydsp_last_pulled_extras_hash`) · a 1.2.163 extras timestamp with no list hash still flushes · lastPulledHoldingIds with no holding hashes still overlays an unpushed qty · no pulled scalar hashes still keeps staking / FIRE after REPLACE · Unlock / `doPull` / `applyReviewedPull` / satellite `runOneButtonSync` stamp `lastPulledHoldingIds` from Mini’s remote (`stampLastPulledHoldingIdsFromRemote`) so an unpushed ETH / Kids book / SOL delete still flushes · first Unlock still drops leftovers · `#F7931A` + Mini-as-book kept
- Mini absorb holding sizes (v1.2.163): book `pushSync` merges satellite BTC qty/cost (and other holding edits) before PUT so Mini boot / Backup / sitting absorb never revert a MacBook / iPhone / iPad size change · extras-only Mini dirty still takes the satellite book (last-stamp) so a new Mini channel PUTs instead of parking · park only when both sides changed the same rows from that stamp · extras absorb (1.2.160) unchanged · sitting satellite pull (1.2.162) unchanged · `#F7931A` + Mini-as-book kept
