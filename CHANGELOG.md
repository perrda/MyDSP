...
- **Overlay after REPLACE:** `overlayMiniBookAfterRemoteReplace` restores Mini-only new books (`snapshotMiniCreatedBooks`), drops Mini deletes (`dropMiniDeletedBooks`), restores Mini renames Mini actually made (`restoreMiniRenamedBooks` vs last registry stamp — never revert a satellite Kids → Children), and overlays Mini-edited scalars vs `mydsp_last_book_scalar_hashes`. `overlayMiniLiveMarks` keeps Mini’s live crypto/equity prints so a MacBook qty does not snap Today back to an old envelope price; absorb then `refreshLiveMarksAfterUnlock`. A satellite dirty pull-then-push does the same for staking / FIRE / budgets (`overlaySatelliteNonCollectionFields` vs `mydsp_last_pulled_scalar_hashes`) so REPLACE does not wipe an iPad reward then upload Mini’s empty list. `overlayDirtyLocalHoldings` keeps satellite qty / new rows and Mini-only adds, and drops rows Mini deleted so a MacBook channel edit does not resurrect SOL. Satellite qty / new ETH / deleted SOL still land. A MacBook YouTube channel still absorbs. Rename marks the book dirty.
