...
- **Yahoo / CoinCap last print:** Yahoo uses the holdings CORS-proxy stack (weekend last close counts). CoinCap walks CORS-ok CoinCap URLs (`graphql.coincap.io` asset priceUsd, then v2) until a numeric BTC last — a CoinGecko holdings fill does not count as CoinCap sampled. Missing Finnhub key stays a skip.
