...
- **Both sides edited:** if Mini is dirty and the same holdings also changed on the satellite, extras still apply and the PUT is skipped (`parked`) — `applyMergePreview` only when Mini is dirty and there is no same-row fight. never revert either book. Open Settings → Sync to review.
