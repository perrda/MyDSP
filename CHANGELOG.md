...
- **Overlay after REPLACE:** `overlayMiniBookAfterRemoteReplace` restores Mini-only new books (`snapshotMiniCreatedBooks`), drops Mini deletes (`dropMiniDeletedBooks`), restores Mini renames Mini actually made (`restoreMiniRenamedBooks` vs last registry stamp — never revert a satellite Kids → Children), and overlays Mini-edited scalars vs `mydsp_last_book_scalar_hashes`. A satellite dirty pull-then-push does the same for staking / FIRE / budgets (`overlaySatelliteNonCollectionFields` vs `mydsp_last_pulled_scalar_hashes`) so REPLACE does not wipe an iPad reward then upload Mini’s empty list. Satellite qty / new ETH / deleted SOL still land. A MacBook YouTube channel still absorbs. Rename marks the book dirty.
