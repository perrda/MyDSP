...
- **Why a Mini Kids book or staking reward could vanish:** 1.2.163 takes the satellite book (`applyRemoteAsBook`) when Mini has not edited the same holding rows. That REPLACE dropped a Kids book Mini just created, undid a Mini delete or Kids → Children rename, and wiped staking / FIRE / budget scalars Mini edited — then PUT that reverted book.
- **Overlay after REPLACE:** `overlayMiniBookAfterRemoteReplace` restores Mini-only new books (`snapshotMiniCreatedBooks`) and already-pushed Mini books / holdings / staking / renames a stale satellite never pulled or edited (`restoreMiniKeptBooks` vs envelope `lastPulledBookIds`, `restoreMiniKeptHoldings` vs `lastPulledHoldings`, `restoreMiniKeptScalars` vs `lastPulledScalarHashes`, `restoreMiniKeptNames` vs `lastPulledRegistryNames` — a MacBook extras PUT that Unlock-ed before Kids, ETH, a staking reward, or Kids → Children does not wipe them; a MacBook that pulled Kids / SOL and deleted it, or edited staking / renamed, still lands), drops Mini deletes (`dropMiniDeletedBooks`), restores Mini renames Mini actually made (`restoreMiniRenamedBooks` vs last registry stamp — never revert a satellite Kids → Children), and overlays Mini-edited scalars vs `mydsp_last_book_scalar_hashes`. `lastPulledHoldingIds` also lives in `mydsp_last_pulled_holding_ids` so a Settings save cannot wipe the envelope ids a stale PUT needs. `overlayMiniLiveMarks` keeps Mini’s live crypto/equity prints so a MacBook qty does not snap Today back to an old envelope price; absorb then `refreshLiveMarksAfterUnlock`. After `applyMergePreview`, `dropUneditedRowsDeletedOnRemote` drops a satellite SOL delete that local-first pickById would union back when Mini only edited BTC. `dropMiniDeletedHoldings` drops a SOL Mini already deleted so REPLACE / merge does not put it back when the MacBook only changed BTC. A satellite dirty pull-then-push does the same for staking / FIRE / budgets (`overlaySatelliteNonCollectionFields` vs `mydsp_last_pulled_scalar_hashes`) so REPLACE does not wipe an iPad reward then upload Mini’s empty list. `overlayDirtyLocalHoldings` keeps satellite qty / new rows and Mini-only adds, and drops rows Mini deleted so a MacBook channel edit does not resurrect SOL. Unlock / boot / sitting pull use the same overlay via `overlaySatelliteBookAfterRemoteReplace` when `satelliteBookDivergedFromLastPull` (hashes in `mydsp_last_pulled_holding_hashes`, not the in-memory dirty flag lost on reload) so an unpushed MacBook qty survives Unlock — first Unlock (no stamp) still REPLACE leftover DAVID. The same reload hole for YouTube / News / Markets uses `satelliteExtrasDivergedFromLastPull` (`mydsp_last_pulled_extras_hash` — channel / tag / ticker ids only) so an unpushed MacBook channel still flushes after Unlock. `applyWorkspaceExtrasFromPreview` (Unlock, sitting pull, Settings media pull, Advanced Pull) marks that diverge dirty so Mini absorb still gets it. A 1.2.163 satellite with `lastWorkspaceExtrasSyncAt` but no list hash is treated as diverged so the first 1.2.164 Unlock still flushes an unpushed channel (first Unlock with no extras stamp still REPLACE leftover DAVID). The same upgrade path for sizes: `lastPulledHoldingIds` with no `mydsp_last_pulled_holding_hashes` is diverged so an unpushed MacBook qty overlays instead of snapping back to Mini. `overlaySatelliteNonCollectionFields` with no `mydsp_last_pulled_scalar_hashes` keeps local staking / FIRE / budgets after REPLACE so a 1.2.163 iPad reward is not wiped then pushed. Unlock / `doPull` stamp Mini’s book hashes before overlay so pull-then-push does not treat the kept reward as already pulled; `stampLastPulledHoldingIdsFromRemote` writes Mini’s remote ids after overlay (not the overlaid ETH / Kids / missing SOL) so a Mini-deleted SOL still drops and the next pull-then-push still flushes an unpushed add or satellite SOL delete; `applyReviewedPull` (Advanced Pull / file import) uses the same overlay + remote-id stamp so Settings **Pull book from Mini** does not wipe an unpushed ETH; satellite `runOneButtonSync` calls `unlockAndPullFromCloud` (never a bare REPLACE); a satellite PUT then stamps the uploaded book. Mini + MacBook + iPad sizes, YouTube / News / Markets, and FX land both ways. Satellite qty / new ETH / deleted SOL still land. A MacBook YouTube channel still absorbs. Rename marks the book dirty.
- **Keep:** Mini absorb holding sizes (1.2.163), sitting satellite extras pull (1.2.162), live-price trust (1.2.161), Mini absorb extras (1.2.160), Mini-as-book, `#F7931A`. Draft only — do not Promote / wrangler / merge #206. Service worker cache is `mydsp-v1.2.164`.
...
- **Absorb the book too:** `absorbRemoteWorkspaceExtrasBeforePush` still unions extras. When Mini has not edited, it `applyRemoteAsBook`s the satellite envelope so a changed qty, new ETH, or deleted SOL is not unioned back to yesterday. Mini open + Automatic off uses the same path on focus / 60s / online. A satellite edit flush still REPLACE-pulls, then `overlayDirtyLocalHoldings` keeps local qty / newly added rows / deletes and Mini-only new ids so pull-then-push does not upload Mini’s older book. Advanced Pull / Unlock / one-button REPLACE stamp `lastPulledHoldingIds` so a later delete is not treated as a Mini-only add. A portfolio created on the satellite after Unlock is snapshotted and restored after REPLACE (`restoreSatelliteCreatedBooks`) so Kids / a new sleeve is not dropped before the push. A portfolio deleted on the satellite after Unlock is dropped again after REPLACE (`dropSatelliteDeletedBooks`) so Mini absorb does not resurrect it. A rename (Kids → Children) is restored after REPLACE (`restoreSatelliteRenamedBooks`).
- **Both sides edited:** park only when Mini and the satellite both changed the same rows from Mini’s last book stamp (`mydsp_last_book_holding_hashes`). Extras-only Mini dirty (new YouTube / News / Markets, holdings still match the stamp) still `applyRemoteAsBook` so a MacBook qty is not parked and Mini’s new channel still PUTs. Mini qty + satellite extras-only keeps Mini’s size and absorbs the channel. `applyMergePreview` for mixed Mini-only / satellite-only rows. never revert either book. Open Settings → Sync only when both sides edited the same holding.
...
- **Draft hosts:** `pushSync` is blocked on leftover `cursor-*-mydsp` (no v1) and commit previews like `047722a6-mydspv1…`, not only `cursor-*-mydspv1`. Live and `main-mydspv1` still push. A failed preview apply no longer stamps `lastRemoteExportedAt` so the next minute can retry.
...
- **Mini stays open:** Automatic off used to skip the 60s / focus cycle. Mini then never read a satellite extras push until reload or Backup. Focus / interval / online now absorb and PUT only when the other device wrote extras (or Mini is dirty) — no empty 60s upload.
...
- **Advanced Pull / import:** leftover DAVID always creates conflicts. A satellite no longer parks the conflict sheet and skip REPLACE — `Pull book from Mini`, `.enc.json` import, `pullAndMerge`, and `importEncryptedFile` call `applyReviewedPull` even when `conflicts.length > 0`. Mini still reviews. Settings media pull refreshes live marks after extras.
...
- **Why leftovers could still overwrite Mini:** Unlock / boot / main Sync already REPLACE on a satellite. Settings Advanced **Pull & merge**, **Apply merge**, `.enc.json` import, and the conflict sheet still called `applyMergePreview`, which unions leftover DAVID / extra portfolios. After `lastSyncAt` the next satellite push could send that mix back to Mini. Advanced **Rotate passphrase** also `pushSync`d on a satellite.
- **Replace:** those paths call `applyReviewedPull` — satellites `applyRemoteAsBook` (drop leftover holdings and extra books) then refresh live FX / marks. Mini (book) still reviews and unions. Button on a satellite reads **Pull book from Mini**. Rotate passphrase is Mini-only.
- **Draft previews:** `cursor-*-mydspv1` hosts must not `pushSync` — that origin is empty and would overwrite Mini’s cloud book. Use `main-mydspv1` or Live.
- **Mini Live:** `go-live.sh` prints the Live service-worker version after deploy and stops with a `wrangler login` hint if Cloudflare is not signed in.
...
- **Add on any device:** after unlock, a satellite YouTube / News / Markets edit pull-then-pushes extras so Mini receives them. Mini Backup now / daily backup also pushes the envelope when the passphrase is unlocked — Automatic sync can be off. Satellites never push from Backup.
- **Remembered passphrase is not a pull:** Unlock banner stays until `lastSyncAt`. App start on a MacBook / iPad with the passphrase remembered pulls Mini even if Automatic is off. Advanced Push and offline-queue flush on a satellite pull only. A satellite never seeds an empty cloud. Markets same-symbol rows last-write-wins so Mini notes land.
...
- **SIPP = Equities:** Today’s SIPP row is the priced equity sleeve (`calcEquity`) — TSLA / MSTR / every included live-quoted equity on DAVID (and every other book). It is not filtered to `accountType === 'sipp'`. That is why live 1.2.132 showed SIPP USD 0.00 while Net Worth already included those names.
- **Same number:** Equities page VALUE and goal metric `sipp` use `calcEquity`. Unpriced stay excluded. SIPP still opens `/equities`. No `/sipp` route. Account-type chips (ISA / SIPP / general) on a holding stay as labels only.
...
- **Today hero:** the bold figure is **Assets** (not Net Worth). The NW change-since-yesterday line and 7D/30D trend stay. Under the spark: Net Worth (`/money`), Crypto Assets (`/crypto`), SIPP (`/equities` — no `/sipp` route; SIPP is the equity sleeve), Liabilities (`/liabilities`). Book numbers only. Cash runway stays.
...
### Added — Provider health ping-all on header Refresh (draft, no live)
...
- **This-tick sources:** Header Refresh records provider health from the `source` on quotes already fetched by `refreshPrices` / `refreshFx`. If CoinGecko just priced holdings, it is OK — no second `simple/price`. Empty / undefined last does not count as success.
- **Yahoo / CoinCap last print:** Yahoo and CoinCap walk CORS-ok URLs in parallel until a numeric last (weekend last close counts). Yahoo uses `r.jina.ai` then the holdings proxy stack; CoinCap keeps `graphql.coincap.io` asset priceUsd. Empty / blocked bodies try the next URL — they are not a skip. A CoinGecko holdings fill does not count as Yahoo or CoinCap sampled. Missing Finnhub key stays a skip.
...
### Fixed — Unlock hydrate, Mini live quotes, phone News/YouTube, satellite PTR (draft, no live)
- **Unlock Sync:** After a working Mini or satellite session, refresh hydrates Remember + last-sync. The header chip shows Synced / last-sync — not amber Unlock — when a remembered passphrase exists. Opening Settings is not required to dismiss Unlock.
