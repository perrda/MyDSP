...
- **Yahoo / CoinCap last print:** Probes use the same CORS-proxy stack as equity holdings (`holdingsQuoteProxyCandidates` / `fetchViaHoldingsProxies`). Weekend last close counts. Empty or CORS-blocked bodies try the next proxy and skip (not fail) if every holdings relay is empty. Missing Finnhub key stays a skip.
