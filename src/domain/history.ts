...
export function formatChartTick(isoDate: string, range: ChartRange, at?: string): string {
  if (range === '1D' && at) {
    return at.slice(11, 16) // HH:MM
  }
  if (range === '1D' || range === '1W' || range === '1M') return isoDate.slice(5)
  if (range === 'YTD' || range === '12M') return isoDate.slice(5)
