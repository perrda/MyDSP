...
describe('MyDSP 1.2.122 cashflow', () => {
  ...
  it('reuses recurring, spending, budgets, and runway — no second ledger', () => {
    ...
    expect(domain).toMatch(/from '\.\/goalProjectedDate'/)
    expect(domain).toMatch(/estimateMonthlyExpenses/)
    expect(domain).toMatch(/calcCash/)
    expect(domain).toMatch(/monthlyEquivalent/)
  ...
  it('Today keeps one quiet runway line that opens Cashflow', () => {
    ...
    expect(dash).toMatch(/monthlyRecurringTotal/)
    expect(dash).toMatch(/calcCash/)
