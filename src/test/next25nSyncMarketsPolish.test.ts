...
describe('next25n — sync / Markets / Today polish tip (1–25 → v1.2.80)', () => {
  ...
  it('25: package + release notes are 1.2.80', () => {
    ...
    expect(pkg.version).toBe('1.2.119')
    expect(RELEASE_NOTES[0]?.version).toBe('1.2.119')
  ...
  it('11–15: mobile Notify · Favourites long-press · PTR · thumb · YT datetime', () => {
    ...
    expect(history).toMatch(/thumb-cta-bar|page-primary-actions|page-primary-create|PagePrimaryActions/)
    expect(budgets).toMatch(/thumb-cta-bar|page-primary-actions|page-primary-create|PagePrimaryActions/)
    expect(imp).toMatch(/thumb-cta-bar|page-primary-actions|page-primary-create|PagePrimaryActions/)
