...
describe('MyDSP 1.2.164 Mini absorb keeps Mini family books and staking', () => {
  it('package + release notes tip', () => {
    ...
    expect(section).toMatch(/restoreMiniKeptHoldings/)
    expect(section).toMatch(/restoreMiniKeptScalars/)
    expect(section).toMatch(/restoreMiniKeptNames/)
    expect(section).toMatch(/already-pushed/)
    expect(section).toMatch(/lastPulledBookIds/)
    expect(section).toMatch(/lastPulledHoldings/)
    expect(section).toMatch(/lastPulledScalarHashes/)
    expect(section).toMatch(/lastPulledRegistryNames/)
    expect(section).toMatch(/mydsp_last_pulled_holding_ids/)
    ...
    expect(notes).toMatch(/Kids, ETH, staking reward, or Kids → Children Mini already pushed/)
    ...
    expect(releaseBulletText(kids)).toMatch(/Kids, ETH, staking reward, or Kids → Children Mini already pushed/)
  ...
  it('absorb REPLACE overlays Mini registry and scalars', () => {
    ...
    expect(absorbFn).toMatch(/lastPulledHoldings/)
    expect(absorbFn).toMatch(/lastPulledScalarHashes/)
    expect(absorbFn).toMatch(/lastPulledRegistryNames/)
    expect(read('../services/sync/syncService.ts')).toMatch(/restoreMiniKeptBooks/)
    expect(read('../services/sync/syncService.ts')).toMatch(/export function restoreMiniKeptHoldings/)
    expect(read('../services/sync/syncService.ts')).toMatch(/export function restoreMiniKeptScalars/)
    expect(read('../services/sync/syncService.ts')).toMatch(/export function restoreMiniKeptNames/)
    expect(read('../services/sync/syncService.ts')).toMatch(/mydsp_last_pulled_holding_ids/)
    expect(read('../services/sync/syncService.ts')).toMatch(/export function loadLastPulledHoldingIds/)
    ...
    expect(rule).toMatch(/restoreMiniKeptHoldings/)
    expect(rule).toMatch(/restoreMiniKeptScalars/)
    expect(rule).toMatch(/restoreMiniKeptNames/)
    expect(rule).toMatch(/lastPulledHoldings/)
    expect(rule).toMatch(/lastPulledScalarHashes/)
    expect(rule).toMatch(/lastPulledRegistryNames/)
    expect(rule).toMatch(/mydsp_last_pulled_holding_ids/)
