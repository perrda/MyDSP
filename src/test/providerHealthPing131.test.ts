...
describe('MyDSP 1.2.131 provider health ping-all', () => {
  ...
  it('header Refresh invokes ping-all after refreshPrices / refreshFx', () => {
    ...
    expect(health).toMatch(/yahooAndCoinCap/)
    expect(health.indexOf("run('yahoo'")).toBeLessThan(health.indexOf("run('coincap'"))
  ...
  it('probes reuse existing quote helpers and honour CoinGecko backoff', () => {
    ...
    expect(prices).toMatch(/YAHOO_AAPL_CORS_URLS/)
    expect(prices).toMatch(/r\.jina\.ai/)
    expect(prices).toMatch(/holdingsQuoteProxyCandidates\(/)
