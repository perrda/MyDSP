...
describe('MyDSP 1.2.131 provider health ping-all', () => {
  ...
  it('header Refresh invokes ping-all after refreshPrices / refreshFx', () => {
    ...
    const prices = shell.indexOf('await refreshPrices()')
    const fx = shell.indexOf('await refreshFx()')
    const ping = shell.indexOf('pingAllMarketsProviders')
    expect(prices).toBeGreaterThan(-1)
    expect(fx).toBeGreaterThan(prices)
    expect(ping).toBeGreaterThan(fx)
    const toast = shell.indexOf('Updated ${r.crypto} crypto')
  ...
  it('probes reuse existing quote helpers and honour CoinGecko backoff', () => {
    ...
    expect(prices).toMatch(/fetchEquityMarketQuote\('AAPL', ''\)/)
